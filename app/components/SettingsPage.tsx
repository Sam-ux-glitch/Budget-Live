...
export default function SettingsPage({supabase,userId,categories}:{supabase:SupabaseClient;userId:string;categories:{name:string}[]}){
 ...
 <SettingsSection title="Widget settings"><p className="text-zinc-400 mb-3">Choose up to four categories for this device. Their remaining amounts appear in the Home Screen widget and optional Lock Screen widget. Uncheck all to hide them. Open the app to refresh; iOS controls widget refresh timing.</p>
 {categories.map(c=><label key={c.name} className="flex items-center gap-3 py-2"><input type="checkbox" checked={selected.includes(c.name)} disabled={!selected.includes(c.name)&&selected.length>=4} onChange={e=>{const next=e.target.checked?[...selected,c.name]:selected.filter(n=>n!==c.name);setSelected(next);localStorage.setItem('budget-live-widget:'+userId,JSON.stringify(next));setStatus('Saved for this device.');void refreshWidget(supabase,userId).catch(()=>setStatus('Saved. Open the app again to retry updating the widget.'));}}/>{c.name}</label>)}{status&&<p role="status">{status}</p>}</SettingsSection>
