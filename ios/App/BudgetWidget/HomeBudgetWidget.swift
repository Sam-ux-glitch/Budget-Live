        ...
        VStack(alignment: .leading, spacing: large ? 10 : 4) {
            HStack(alignment: .firstTextBaseline) {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("Left this month").font(.system(size: large ? 20 : 14, weight: .bold))
                }
                Spacer()
            ...
            if let snapshot = entry.snapshot, snapshot.isCurrent(at: entry.date), !snapshot.categories.isEmpty {
                ForEach(snapshot.categories) { row in
                    VStack(alignment: .leading, spacing: large ? 4 : 2) {
                        HStack {
                            Text(row.name).font(.system(size: large ? 14 : 11, weight: .medium)).lineLimit(1).minimumScaleFactor(0.8)
                            Spacer(minLength: 6)
                            Text(row.remaining, format: .currency(code: "USD").precision(.fractionLength(0)))
                                .font(.system(size: large ? 20 : 14, weight: .bold)).monospacedDigit()
                        ...
                        }.frame(height: large ? 6 : 4)
                        .accessibilityLabel(row.name + " remaining " + String(Int(row.progress * 100)) + " percent")
                        if large {
    Text(
        row.remaining < 0
        ? "Over budget"
        : "of " + row.budget.formatted(.currency(code: "USD").precision(.fractionLength(0)))
    )
    .font(.caption2)
